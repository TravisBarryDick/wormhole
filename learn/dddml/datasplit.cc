...
*/
  ...
  std::vector < FeaID > *Intersect(std::vector < FeaID > *v1,
                                   std::vector < FeaID > *v2) {
    // features do not exist
    if (v1->size() == 0)
      return new std::vector < FeaID > (*v2);
    else if (v2->size() == 0)
      return new std::vector < FeaID > (*v1);
    //else:
  ...
  void subsample(const char *featureFile,       //name of feature file
    ...
    std::
      cerr << "Probability of selecting a row: " <<
      probability_of_selecting_one_row << std::endl;
    probability_of_selecting_one_row =
      (probability_of_selecting_one_row >
       1.0) ? 1.0 : probability_of_selecting_one_row;
    ...
    for (int fi = 0; fi < nFiles; ++fi) {
      for (int part = 0; part < nPartToRead; ++part) {
        ...
        MinibatchIter < FeaID > reader(filename, partID, nPartPerFile,
                                       data_format, mb_size);
