...
*/
  ...
  template < typename I >
    ...
    bool changed = true;
    vector_vector_int_ptr assignments =
      std::make_shared < std::vector < std::vector < int >>>(data.size,
                                                             std::vector <
                                                             int >(p));
    start = GetTime();
    int iter_count = 0;
...
#if DISTRIBUTED
    LOG(INFO) << "Finished k-means: " << iter_count << " iterations in " << time
      << " sec";
#else
    std::
      cout << "Finished k-means: " << iter_count << " iterations in " << time <<
      " sec" << std::endl;
#endif

    return std::pair < vector_vector_int_ptr, centers_t > (assignments,
                                                           centers);
  }
...
/* p > 1 */
  ...
  for (int i:permutation) {
      if (counts[i] >= 0 && counts[i] < current_lower) {
        ...
        --current_k;
        std::
          cout << "Merged cluster " << i << " with cluster " << new_index <<
          std::endl;
        int counter = 0;
        for (int j = 0; j < assignments->size(); ++j) {
          for (int h = 0; h < p; ++h) {
            if ((*assignments)[j][h] == i) {
              (*assignments)[j][h] = new_index;
            }
          }
        }
        //update centers:
  ...
  for (int i:permutation) {
      if (counts[i] >= 0 && counts[i] > current_upper) {
        ...
        std::
          cout << "split cluster " << i << " to clusters " << current_index <<
          " ... " << (current_index + num_new_clusters_for_this_cluster -
                      1) << std::endl;
    ...
    for (int i = 0; i < assignments->size(); ++i) {
      for (int j = 0; j < p; ++j) {
        ...
        if (to_be_swapped[temp])
          (*assignments)[i][j] = swap_map[temp];
      }
    }
...
#else
    std::
      cout << "Merged: " << nmerge << " and split " << nsplit << ".\n Old k: "
      << k << "; Current k: " << current_k << std::endl;
#endif
    return current_k;
  }
...
using myPair =
  std::pair < std::vector < FeaID > *,
  dmlc::data::RowBlockContainer < FeaID > *>;
...
myPair readSamplingOutput(const char *filename)
{
  ...
  dmlc::data::RowBlockContainer < FeaID > *data =
...
int main(int argc, char *argv[])
{
  ...
  Stream *num_clusters_out =
    Stream::Create(cfg.dispatched_num_clusters_path().c_str(), "w");
...
int main()
{
  ...
  dmlc::data::RowBlockContainer < int >rbc =
    dmlc::data::libsvmread("./mnist.txt");
